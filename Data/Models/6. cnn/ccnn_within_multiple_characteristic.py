"""
This script trains a Compact Convolutional Neural Network (CCNN) using the within-subject approach,
for multiple EEG characteristics per electrode (i.e., 24 input channels).

Each subject has their own training/testing split. 10-fold cross-validation is used to select the best model,
which is then evaluated on the test set.

Data input shape: (batch_size, 24, 9, 9)  → 24 features (6 per electrode * 4 electrodes) in a 9x9 spatial grid.
"""
...
approach_output_dirs = {
    "3": "Results/1. with_in_subject/b. multiple_characteristic/slice_wise/6. cnn",
    "4": "Results/1. with_in_subject/b. multiple_characteristic/trial_wise/6. cnn",
}
...
def ccnn_within_model_multiple(approach: str):
    """Main function to train CCNN for within-subject setting with multiple EEG features per channel."""
    ...
    for subject, (X_train, X_test, y_train, y_test) in all_data.items():
        ...
        for fold, (train_idx, val_idx) in enumerate(kfold.split(X_train)):
            ...
            # NOTE: 24 input channels due to multiple characteristics per electrode
